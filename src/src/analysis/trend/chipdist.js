// ref: copy from eastmoney.com webpack://quotechart2022/src/modules/tools/indicator/cyq.ts
// modified
 ...
 */
export function ChipDistCalculator(kdata, accuracyFactor, range) {
   /**
...
*/
ChipDistCalculator.prototype.calc = function (index) {
   var maxprice = 0;
   var minprice = 0;
    ...
    * K图数据[time,open,close,high,low,volume,amount,amplitude,turnoverRate]
    */
   var kdata = this.klinedata.slice(
      start, Math.max(1, index + 1)
   ).filter(
      z => z.V > 0 && z.s !== null && !isNaN(z.s)
   );
   if (kdata.length === 0) throw 'invaild index';
   for (var i = 0; i < kdata.length; i++) {
       var elements = kdata[i];
       maxprice = !maxprice ? elements.H : Math.max(maxprice, elements.H);
       minprice = !minprice ? elements.L : Math.min(minprice, elements.L);
   }
   ...
   for (var i = 0; i < kdata.length; i++) {
       ...
       var open = eles.O,
           ...
           turnoverRate = Math.min(1, eles.s / 100 || 0);
   ...
   var currentprice = this.klinedata[index].C;
   var totalChips = 0;
   for (var i = 0; i < factor; i++) {
